const resolveSceneText = (scene, ctx) => typeof scene.text === "function" ? scene.text(ctx) : scene.text;
        ...
        cutscenes: [
            ...
            },
            {
                ...
                text: (ctx) => [
                    ...
                    `You just turned ${ctx.startCapital.toFixed(0)} into ${ctx.portfolioValue.toFixed(0)}. The board just noticed.`,
                ],
            },
            {
                id: "ACT_I_FIRST_RED",
    ...
    },
    {
        ...
        cutscenes: [
            ...
            },
            {
                ...
                trigger: "day",
                once: true,
                condition: (ctx) => ctx.activeWhaleId != null && ctx.portfolioValue < ctx.startCapital && ctx.day > 3,
                text: [
...
const hasSeenCutscene = (sceneId, seenCutsceneIds) => {
    if (!seenCutsceneIds)
        return false;
...
export function getTriggeredCutscenes(ctx, trigger, seenCutsceneIds) {
    const result = [];
...
export function renderSceneText(scene, ctx) {
