...
export const initializeUI = (runner, container, options = {}) => {
  ...
  `;
    ...
    const marketSummaryGrid = container.querySelector("[data-role='market-summary-grid']");
    ...
    const eraStatus = container.querySelector("[data-role='era-status']");
    const eraStatusName = container.querySelector("[data-role='era-name']");
    const eraStatusProgress = container.querySelector("[data-role='era-progress']");
    const holdingsList = container.querySelector("[data-role='holdings-list']");
    const holdingsSortSelect = container.querySelector("[data-role='holdings-sort']");
    ...
    const cheatSheetModal = document.querySelector("[data-role='cheat-sheet-modal']");
    const cheatSheetCurrent = document.querySelector("[data-role='cheat-sheet-current']");
    const cheatSheetClose = document.querySelector("[data-action='close-cheat-sheet']");
    const sideHustleModal = document.querySelector("[data-role='side-hustle-modal']");
    const sideHustleModalTitle = document.querySelector("[data-role='side-hustle-modal-title']");
    const sideHustleModalSubtitle = document.querySelector("[data-role='side-hustle-modal-subtitle']");
    const sideHustleModalStory = document.querySelector("[data-role='side-hustle-modal-story']");
    const sideHustleModalPrompt = document.querySelector("[data-role='side-hustle-modal-prompt']");
    const sideHustleModalClose = document.querySelector("[data-action='close-side-hustle-modal']");
    ...
    const devForceSaveButton = container.querySelector("[data-action='dev-save-update']");
    const devForceResetButton = container.querySelector("[data-action='dev-save-reset']");
    ...
    let sideHustleHighlightTimer = null;
    const highlightSideHustleCard = () => {
        if (!sideHustleCard)
            return;
        ...
        const stats = [
            {
                ...
                detail: `Cash ${formatCurrency(runner.state.portfolio.cash)} · Holdings ${formatCurrency(Math.max(0, latest - runner.state.portfolio.cash))}`,
            },
            {
    ...
    const refreshHoldings = () => {
        refreshHoldingsPanel(holdingsList, runner.state.portfolio, runner.state.companies, selectedTicker, handleHoldingSelect, holdingsSortMode);
    };
    ...
    const viewMenu = container.querySelector(".view-menu");
    ...
    const viewMenuToggle = container.querySelector("[data-action='toggle-view-menu']");
    const viewMenuList = container.querySelector("[data-role='view-menu-list']");
    let menuOpen = false;
    ...
    const formatEraCheatSection = (eraId) => {
    ...
    const renderCommunityIncomeStats = () => {
        if (!communityIncomeStats)
            return;
        const state = runner.state.dca;
        const stream = DCA_STREAMS[state.activeStreamId];
        const investedText = state.totalContributed > 0
      ...
      <p class="local-income-panel__stats-detail">${investedText} · Earned ${formatCurrency(state.totalEarned)}</p>
    `;
    };
    const renderDcaOffers = () => {
        if (!dcaOffersContainer)
            return;
        const state = runner.state.dca;
    ...
    const renderDcaEventLog = () => {
        ...
        for (const entry of entries) {
            ...
            const deltaStr = entry.cashDelta === 0
                ? ""
                : ` (${entry.cashDelta >= 0 ? "+" : "-"}${formatCurrency(Math.abs(entry.cashDelta))})`;
    ...
    holdingsSortSelect?.addEventListener("change", () => {
        ...
        const nextMode = ["ticker", "value", "delta"].includes(nextValue)
            ? nextValue
            : "ticker";
