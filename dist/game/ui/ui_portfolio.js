...
export const refreshHoldingsPanel = (container, portfolio, companies, selectedTicker, onSelect, sortMode = "ticker") => {
    if (!container)
        return;
    container.innerHTML = "";
    const rawEntries = Object.entries(portfolio.holdings).filter(([, quantity]) => quantity > 0);
    const entries = rawEntries.map(([ticker, quantity]) => {
    ...
    for (const entry of entries) {
        ...
        button.textContent = `${entry.ticker}: ${entry.quantity} sh · ${formatCurrency(entry.value)} (${changeLabel})`;
