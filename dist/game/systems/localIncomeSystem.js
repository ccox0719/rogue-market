...
export const localIncomeDefinitions = [
    {
...
export const getLocalIncomeDefinition = (id) => DEFINITIONS_BY_ID.get(id);
export const createInitialLocalIncomeStreams = () => localIncomeDefinitions.reduce((acc, definition) => {
    acc[definition.id] = { activeEvent: null };
    return acc;
}, {});
const pickRandom = (items, rng) => items[Math.floor(rng.next() * items.length)];
const resolveDuration = (event, rng) => {
...
const applyNegativeEvent = (state, definition, status, eventDef, rng) => {
...
const applyBonusEvent = (state, definition, eventDef, rng) => {
    const message = `${definition.name}: ${eventDef.description}`;
    recordEvent(state, {
        day: state.day,
        streamId: definition.id,
...
export const processLocalIncomeStreams = (state, rng) => {
