...
const resetCompanyRange = (company) => {
    company.todayRange = {
        open: company.price,
        high: company.price,
        low: company.price,
...
const clearWatchOrdersForCompany = (state, companyId) => {
    state.watchOrders = state.watchOrders.filter((order) => order.companyId !== companyId);
};
export const splitCompany = (state, company, ratio, options = {}) => {
    if (!company.isActive) {
        return false;
    }
    if (ratio <= 1) {
...
export const processStockLifecycle = (state, rng) => {
    ...
    for (let i = 0; i < companyCount; i += 1) {
        ...
        const splitThreshold = company.splitReferencePrice *
            CONFIG.SPLIT_THRESHOLD_MULTIPLIER *
            splitMultiplier;
        if (company.price >= splitThreshold &&
        ...
        const bankruptcyThreshold = CONFIG.BANKRUPTCY_PRICE_THRESHOLD * bankruptcySeverity;
