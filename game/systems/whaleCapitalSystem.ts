...
export const updateWhaleCapital = (state: GameState): void => {
  ...
  for (const whale of state.activeWhales) {
    ...
    const newCapital = Math.max(
      whale.capital * growthMultiplier * leverage + manipulationProfit - manipulationBackfire,
      100_000
    );
