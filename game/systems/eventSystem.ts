...
import { generateEvent, type GameEvent } from "../generators/eventGen.js";

const clampChance = (value: number): number => Math.min(1, Math.max(0, value));

export const runDailyEvents = (
  state: GameState,
  rng: RNG,
  eventChance: number,
...
export const calculateEventShock = (
  company: Company,
  events: GameEvent[],
  options?: { negativeImpactMultiplier?: number }
): number => {
  ...
  return events.reduce((total, event) => {
    ...
    const affinity =
      company.eventAffinity[event.sectorAffinity ?? company.sector] ?? 1;
