...
import type { GameState, MarketNewsItem, MarketNewsTopic } from "../core/state.js";
...
const createNewsId = (topic: MarketNewsTopic, suffix: string): string =>
  `${topic}:${suffix}`;

const buildEventNews = (event: GameEvent, day: number): MarketNewsItem => {
...
const buildEraNews = (state: GameState, day: number): MarketNewsItem => {
  ...
  const headline =
    state.currentEraMutated && state.mutationMessage
...
export const generateDailyNews = (state: GameState): MarketNewsItem[] => {
  ...
  const whalesToday = state.activeWhales.filter(
    (whale) => whale.visible && whale.lastActionDay === day
  );
...
export const emitMarketNews = (state: GameState): MarketNewsItem[] => {
  ...
  const unique = candidates.filter(
    (item) => !state.newsEventLog.includes(item.id)
  );
  if (unique.length === 0) {
    return [];
  }
