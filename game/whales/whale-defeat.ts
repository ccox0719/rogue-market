...
import {
  getActiveWhaleInstance,
  removeWhaleFromActiveList,
} from "./whale-state.js";

export type WhaleId = WhaleProfile["id"];

export interface WhaleBuyoutCheck {
  canAttempt: boolean;
  reason: string;
  ratio: number;
  requiredRatio: number;
  whaleId: WhaleId | null;
}
...
export function checkWhaleBuyout(state: GameState): WhaleBuyoutCheck {
  ...
  const historicalCapital =
    active.capitalHistory?.[0] ?? active.capital ?? 1;
...
export function applyWhaleBuyout(state: GameState): boolean {
...
export interface WhaleCollapseCheck {
  shouldCollapse: boolean;
  reason: string;
  collapseThresholdPct: number;
  playerMinRatio: number;
  whaleCapital: number;
  baseCapital: number;
  whaleId: WhaleId | null;
...
export function checkWhaleCollapse(state: GameState): WhaleCollapseCheck {
  const active = getActiveWhaleInstance(state);
  const playerCapital = Math.max(0, portfolioValue(state));

  const base: WhaleCollapseCheck = {
...
export function applyWhaleCollapseIfNeeded(state: GameState): GameState {
