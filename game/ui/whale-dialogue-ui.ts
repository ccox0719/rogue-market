...
let _speakTimer: number | null = null;

export function renderWhaleDialogue(state: GameState): void {
  const list = document.querySelector<HTMLElement>("[data-role='whale-dialogue-list']");
  if (!list) return;
