...
let buyoutHandler: (() => void) | null = null;

export function bindWhaleBuyoutHandler(handler: () => void): void {
  buyoutHandler = handler;
  const btn = document.getElementById("whale-buyout-button") as HTMLButtonElement | null;
  if (!btn) return;
  btn.onclick = () => {
...
export function renderWhaleInfluenceBar(state: GameState): void {
  ...
  const buyoutBtn = document.getElementById("whale-buyout-button") as HTMLButtonElement | null;
  ...
  const whaleCapital = Math.max(
    1,
    active.capital ?? active.capitalHistory?.[0] ?? 1
  );
