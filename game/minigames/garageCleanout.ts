...
type LootType = "cash_small" | "cash_medium" | "cash_big" | "nothing" | "collectible";
...
interface GarageState {
  ...
  failureBox: number | null;
  remainingAttempts: number;
  currentCash: number;
  pendingResult: MiniGameResult | null;
  baseScore: number;
}

const DOUBLE_ATTEMPTS = 4;

const LOOT_SCORE_MAP: Record<LootType, number> = {
  nothing: 0.12,
  cash_small: 0.38,
  cash_medium: 0.6,
  cash_big: 0.72,
...
const DEFAULT_GARAGE_SUBTITLE =
  "Mom and Dad told you the garage sale is a mess. Flip the right crate before you bust the busted one.";
...
function applyHeaderContext(context?: { title?: string; subtitle?: string }): void {
  const titleEl = document.querySelector("#minigame-garage-overlay .minigame-title") as HTMLElement | null;
  if (titleEl) {
    titleEl.textContent = context?.title ?? DEFAULT_GARAGE_TITLE;
  }
  const subtitleEl = document.querySelector(
    "#minigame-garage-overlay .minigame-subtitle"
  ) as HTMLElement | null;
  if (subtitleEl) {
...
export function launchGarageCleanoutMiniGame(
  ...
  const closeBtn = $("minigame-garage-close") as HTMLButtonElement | null;
  const lockInBtn = $("minigame-garage-lockin") as HTMLButtonElement | null;
  ...
  const boxButtons = Array.from(
    document.querySelectorAll<HTMLButtonElement>(".minigame-garage-box")
  );
  ...
  const handleClick = (btn: HTMLButtonElement): void => {
  ...
  if (lockInBtn) {
    lockInBtn.onclick = () => {
      if (!garageState.pendingResult) return;
...
function handleInitialPick(
  btn: HTMLButtonElement,
  labelEl: HTMLElement,
  resultEl: HTMLElement,
  boxButtons: HTMLButtonElement[],
  lockInBtn: HTMLButtonElement | null
): void {
  if (garageState.pickedIndex !== null) {
    return;
  }
...
function pickFailureBox(exclude: number): number {
  const candidates = [0, 1, 2, 3, 4].filter((index) => index !== exclude);
  const choice = candidates[Math.floor(Math.random() * candidates.length)];
  return choice;
}

function computeDoubleScore(): number {
...
function completeResult(overlay: HTMLElement, result: MiniGameResult): void {
...
function cleanupHandlers(): void {
  ...
  const boxButtons = document.querySelectorAll<HTMLButtonElement>(".minigame-garage-box");
  boxButtons.forEach((btn) => {
    btn.onclick = null;
  });
}

function generateGarageLoot(): BoxLoot[] {
  const makeLoot = (type: LootType): BoxLoot => {
    switch (type) {
      case "cash_small":
        return {
          type,
  ...
  const lootTypes: LootType[] = ["cash_big", "collectible", "cash_medium", "cash_small", "nothing"];
